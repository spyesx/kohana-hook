...
class Hook_Core {
	...
	public function do_action($Class, $method, array $params = array()){
	...
	public function do_all_actions($hook_name, array $hook_params = array()){
		if( !isset($this->_actions[$hook_name]) || !is_array($hook_params) ){
			return;
		}
		foreach ($this->_actions[$hook_name] as $action) {
			$action['params'] = array_reverse($action['params'], true);
    		$action['params'][] = $hook_params; 
    		$action['params'] = array_reverse($action['params'], true);
			$this->do_action($action['class'], $action['method'], $action['params']);
		}
	}
